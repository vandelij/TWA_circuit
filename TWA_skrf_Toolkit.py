...
class TWA_skrf_Toolkit:
    ...
    def get_npar_spectrum_explicit_npar_array(self, lengths, npar_array, freq, power=[1,0], phase=[0,0],
                                symetric_mode=True,
                                one_cap_type_mode=False,
                                end_cap_mode=False):
        """
        This function returns the entire power analytic power spectrum calculated from the phase difference in the circuit model
        lengths: list of capacitor lengths
        npar_array: iterable of npar spectrum values at which the P(n||) is desired
        freq: frquency of specturm in Hz
        power: iterable size 2, contains the external port excitation 
        phase: same as above but for the phase 
        """
    ...
    def get_phase_diff_and_PR_straps_1_and_2(self, lengths, freq, power=[1,0], phase=[0,0],
                                symetric_mode=True,
                                one_cap_type_mode=False,
                                end_cap_mode=False):
        """
        This function returns the phase difference in radians and power ratio PR between the first and second straps for a symetric array. 
        lengths: list of capacitor lengths
        freq: frquency of specturm in Hz
        power: iterable size 2 for center_fed_mode=False, 3 for True, contains the external port excitation 
        phase: same as above but for the phase 
        """
    ...
    def error_function_npar_match_low_npar_zero_cancel_image_currents(self, prm):
        ...
        err += npar_error + npar_zero_error + phase_diff_error + power_ratio_error  # Includes error for the power ratio in image current strap, phase diff        
    ...
    def run_differential_evolution_global_op_npar_match_low_npar_zero_cancel_image_currents(self, 
                                            length_bounds,
                                            S11_db_cutouff,
                                            freq, # in Hz
                                            freq_bounds,
   ...
   # TODO was used in paper 1 
    def run_differential_evolution_global_op_npar_match_low_npar_zero_cancel_image_currents_reduce_secondary_peaks(self, 
                                            length_bounds,
                                            S11_db_cutouff,
                                            freq, # in Hz
                                            freq_bounds,
                                            ...
                                            lam2_image_current_mag_op,
                                            target_power_ratio_image_current_op,
                                            npar_array_secondary_peak_cancelation,
                                            epsilon_secondary_peak_cancelation,
                                            strategy='best1bin',
                                            symetric_mode=False,
                                            one_cap_type_mode=False,
                                            end_cap_mode=False,
        ...
        """
        This version of the optimization also aims to try and also match the desired npar performance. both the
        peak location (weighted by alpha) and the peak at zero (weighted by gamma) are added to the cost, as well as canceling the 
        image current phase and power ratio. In addition, the  npar_array_secondary_peak_cancelation gives additional spectral
        loactions where the power P(n||) is desired to be reduced. This is for getting rid of inner secondary peaks. 
        """
        self.i_iter = 0
        self.prms = []
        self.errors = []
        ...
        self.alpha_npar_op = alpha_npar_op
        self.gamma_npar_op = gamma_npar_op
        self.npar_array_secondary_peak_cancelation = npar_array_secondary_peak_cancelation
        self.epsilon_secondary_peak_cancelation = epsilon_secondary_peak_cancelation
        self.target_npar = target_npar # this sets the target npar 
        self.npar_bounds_for_npar_op = npar_bounds
        self.num_npars_for_npar_op = num_npars
        self.lam1_image_current_phase_op = lam1_image_current_phase_op # weight of the image current phase on the cost function 
        ...
        self.target_power_ratio_image_current_op = target_power_ratio_image_current_op # the target power ratio I2^2/I1^2, where I1 is the current in the first strap 
        self.freq_for_npar_op = freq
        res = differential_evolution(self.error_function_npar_match_low_npar_zero_cancel_image_currents_reduce_secondary_peaks, 
                                     bounds=length_bounds, strategy=strategy, workers=workers)
        
        return res

    def error_function_npar_match_low_npar_zero_cancel_image_currents_reduce_secondary_peaks(self, prm):
        """
        This error function uses a new parameter, self.beta_length_op, to add error when the cap lengths are not
        close to eachother, which is beta*sum((length - average length)^2) divided by the square of the average length. 
        """
        ...
        # now, try to cancel power spectrum values at specific
        if self.center_fed_mode == True: 
            power_spectrum_secondary_peak_opt = self.get_npar_spectrum_explicit_npar_array( lengths=prm, 
                                                                                            npar_array=self.npar_array_secondary_peak_cancelation, 
                                                                                            freq=self.freq_for_npar_op, 
                                                                                            power=[1,0,0], 
                                                                                            phase=[0,0,0],
                                                                                            symetric_mode=True,
            ...
            power_spectrum_secondary_peak_opt = self.get_npar_spectrum_explicit_npar_array(lengths=prm, 
                                                                                            ...
                                                                                            one_cap_type_mode=False,
                                                                                            end_cap_mode=False)
        # error for the secondary peak optimization 
        secondary_peak_sum = np.sum(power_spectrum_secondary_peak_opt)/power_max
        error_secondary_peaks = self.epsilon_secondary_peak_cancelation*secondary_peak_sum/self.npar_array_secondary_peak_cancelation.shape[0]              
        ...
        err += npar_error + npar_zero_error + phase_diff_error + power_ratio_error + error_secondary_peaks  # Includes error for the power ratio in image current strap, phase diff        
    ...
    def run_differential_evolution_global_op_npar_match_low_npar_zero_cancel_image_currents_reduce_secondary_peaks_S11_debug(self, 
                                            length_bounds,
                                            S11_db_cutouff,
                                            freq, # in Hz
                                            freq_bounds,
        ...
        self.target_power_ratio_image_current_op = target_power_ratio_image_current_op # the target power ratio I2^2/I1^2, where I1 is the current in the first strap 
        self.freq_for_npar_op = freq
        res = differential_evolution(self.error_function_npar_match_low_npar_zero_cancel_image_currents_reduce_secondary_peaks_S11_debug, 
                                     bounds=length_bounds, strategy=strategy, workers=workers)
        
        return res

    def error_function_npar_match_low_npar_zero_cancel_image_currents_reduce_secondary_peaks_S11_debug(self, prm):
        """
        This error function uses a new parameter, self.beta_length_op, to add error when the cap lengths are not
        close to eachother, which is beta*sum((length - average length)^2) divided by the square of the average length. 
        """
